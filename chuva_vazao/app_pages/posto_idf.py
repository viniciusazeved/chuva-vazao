...
if modo.startswith("Upload"):
    ...
    with st.form("manual_idf"):
        ...
        with col3:
            b = st.number_input("b (constante duração, min)", value=10.0, step=1.0, format="%.2f")
        with col4:
            c = st.number_input("c (expoente duração)", value=0.75, step=0.01, format="%.4f")
        ...
        if submit:
            ...
            st.session_state.idf_params = idf.params_from_convention(
                K, a, b, c, convention="idf_generator",
            )
